...
| Frame | 方案是太怂、太局部，还是太飘、太难落地？ | geju / goudi |
...
其中 `Frame` 阶段特意放了两个方向相反的 skill，用来对冲不同 AI 编程助手的典型偏差：

- **`geju`（格局）主要给 Codex 用**：Codex 有时会过度考虑兼容性、历史包袱和局部实现形状，导致方案格局打不开。`geju` 负责把它从“小心翼翼地补丁化”里拉出来，先看清更干净、更高位的目标模型。
- **`goudi`（苟帝）主要给 Claude Code 用**：Claude Code 有时会把方案想得太大、太飘，愿景很强但第一步不够扎实。`goudi` 负责把宏大判断压实成可执行、可验证、可止损的最小推进。

简单说：`geju` 治过怂，`goudi` 治过飘。一个把格局打开，一个把路踩实。
...
- **减少方案过怂或过飘**：用 `geju` 打开高位判断，用 `goudi` 把宏大方向压成可验证的第一步。
...
这个方案太飘了，帮我用苟帝模式压成可执行、可验证、可止损的第一步
帮我把这个想法设计成一份 PRD
...
| `geju` | 主要给 Codex 用：在方案讨论时打开格局，挑战过度兼容、局部细节陷阱和重构恐惧，输出更高位、更有锋芒的判断 | 治 Codex 的过怂和过度兼容，先看清系统应该成为什么，再讨论迁移、阶段和落地 |
| `goudi` | 主要给 Claude Code 用：在方案过大、过飘或缺少第一步时，把宏大方向压实成最小可验证推进、明确砍掉范围和止损规则 | 治 Claude Code 的过飘和落地不足，保留野心，但先证明它能推进 |
